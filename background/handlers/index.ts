...
export class StorageHandlers {
  ...
  private respondError<T>(
    promise: Promise<T>,
    respond: (response: ErrResponse<T>) => void,
  ) {
    promise
  ...
  setupBindings() {
    ...
    bindingsMessages.getAllBindings.stream.subscribe(
    ...
    bindingsMessages.getBindingsForSite.stream.subscribe(
      async ([{ domain, path }, sender, respond]) => {
        const bindings = await bindingsStorage.getBindingsForSite(domain, path)
        respond(bindings)
      },
    )

    bindingsMessages.getBindingsForDomain.stream.subscribe(
      async ([domain, sender, respond]) => {
        const bindings = await bindingsStorage.getBindingsForDomain(domain)
        respond(bindings)
      },
    )

    bindingsMessages.addBinding.stream.subscribe(
      async ([binding, sender, respond]) => {
        this.respondError(bindingsStorage.addBinding(binding), respond)
      },
    )

    bindingsMessages.updateBinding.stream.subscribe(
      async ([binding, sender, respond]) => {
        this.respondError(bindingsStorage.updateBinding(binding), respond)
      },
    )

    bindingsMessages.upsertBinding.stream.subscribe(
      async ([binding, sender, respond]) => {
        this.respondError(bindingsStorage.upsertBinding(binding), respond)
      },
    )

    bindingsMessages.removeBinding.stream.subscribe(
      async ([id, sender, respond]) => {
        this.respondError(bindingsStorage.removeBinding(id), respond)
      },
    )

    bindingsMessages.changeKey.stream.subscribe(
      async ([{ id, key }, sender, respond]) => {
        this.respondError(bindingsStorage.changeKey(id, key), respond)
      },
    )

    bindingsMessages.moveBindings.stream.subscribe(
      async ([payload, sender, respond]) => {
        this.respondError(
          bindingsStorage.moveBindings(
            new Domain(payload.domain),
            new Path(payload.from),
            new Path(payload.to),
          ),
          respond,
        )
    ...
    bindingsMessages.deleteAllBindings.stream.subscribe(
      async ([, sender, respond]) => {
  ...
  setupDisabledPaths() {
    ...
    disabledPathsMessages.getAllDisabledPaths.stream.subscribe(
      async ([, sender, respond]) => {
    ...
    disabledPathsMessages.queryDisabledPaths.stream.subscribe(
      async ([{ domain, path }, sender, respond]) => {
        const disabledPaths = await storage.query(
          new Domain(domain),
          new Path(path),
        )
        respond(disabledPaths)
    ...
    disabledPathsMessages.disablePath.stream.subscribe(
      async ([{ domain, path }, sender, respond]) => {
        this.respondError(
          storage.addEntry(new Domain(domain), new Path(path)),
          respond,
        )
      },
    )

    disabledPathsMessages.enablePath.stream.subscribe(
      async ([{ domain, path }, sender, respond]) => {
        this.respondError(
          storage.removeEntry(new Domain(domain), new Path(path)),
          respond,
        )
      },
    )

    disabledPathsMessages.togglePath.stream.subscribe(
      async ([{ domain, path }, sender, respond]) => {
        this.respondError(
          storage.togglePath(new Domain(domain), new Path(path)),
          respond,
        )
      },
    )
  ...
  setupEvents() {
    this.bindingsStorage.onPathChange$.subscribe(
      async ({ domain, from, to }) => {
        log.info('onPathChange from background index, renaming disabled path', {
          domain,
          from,
          to,
        })
    ...
    this.bindingsStorage.onDeleted$.subscribe(async (binding) => {
      log.info(
        'onDeleted from background index, checking if disabled paths should be pruned',
        binding,
      )
      const bindings = await this.bindingsStorage.query(
        binding.domain,
        binding.path,
      )
      if (bindings.length === 0) {
        await this.disabledBindingPathsStorage.removeEntry(
          new Domain(binding.domain),
          new Path(binding.path),
        )
      }
    })
  }
