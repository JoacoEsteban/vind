...
export class ResourceMigrator {
  private parser = new ResourceParser(
    getExtensionVersion(),
    SERIALIZATION_JSON_INDENT,
    SERIALIZATION_MINIFY,
  )
  ...
  async exportAllResources() {
    return wrapResultAsync(async () => {
      const [bindings, disabledPaths] = await Promise.all([
        this.bindingChannel
          .getAllBindings()
          .then((bindings) => bindings.map(toBindingDoc)),
  ...
  async importResources(json: string) {
    ...
    const [majorResourceVersion, majorExtensionVersion] = [
      resourceVersion,
      extensionVersion,
    ].map((v) => semver.major(v))

    if (majorResourceVersion > majorExtensionVersion) {
      // TODO implement version handling migration
      return Err(
        new ImportedResourceVersionError(resourceVersion, extensionVersion),
      )
    }
    ...
    return wrapResultAsync(() =>
      Promise.all([
        ...
        ...disabledPaths.map((domain_path) =>
          this.disabledPathsChannel.disablePath(
            new Domain(domain_path),
            new Path(domain_path),
          ),
        ),
      ]),
    )
  ...
  async wipeResources() {
    return wrapResultAsync(async () => {
      await Promise.all([this.bindingChannel.deleteAllBindings()])
    })
  }
}
